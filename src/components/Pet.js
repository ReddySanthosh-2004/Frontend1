...
const Pet = () => {
  ...
  const fetchPets = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/pets?category=${type}`);
  ...
  const handleAdopt = async (petId) => {
    if (window.confirm(`Are you sure you want to adopt this ${type}?`)) {
      try {
        await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/pets/${petId}`);
